...
contract TestChainlinkLive is Script {
    ...
    function run() external {
        ...
        bytes memory data2 = abi.encode(
            ETH_USD_FEED, int256(10000 * 10 ** 8), uint8(IOracleConditionResolver.ComparisonOp.LessThan), uint256(3600)
        );
        ...
        bytes memory data4 = abi.encode(
            LINK_USD_FEED, int256(100 * 10 ** 8), uint8(IOracleConditionResolver.ComparisonOp.NotEqual), uint256(3600)
        );
